...
{{--@endforeach--}}
{{--        {{ Auth::id() }}--}}
{{--{{ dd("Sender",$senderMsgs,"receiver",$receiverMsgs) }}--}}

{{--{{ dd($currentUser->receivedMessages($userToChat["id"])) }}--}}
{{--{{ dd($currentUser->messages($userToChat["id"])) }}--}}

{{--{{ $userIds = [] }}--}}

{{--@php($userIds = [])--}}

{{--@foreach($users->map->only("id") as $element)--}}
    {{--    @php($userIds[] = $element["id"])--}}
    {{--    {!! $userIds[] = $element["id"] !!}--}}
{{--        {{ $element }}--}}
    {{--    @php--}}
    {{--        $userIds[] = $element["id"];--}}
...
@php
    $usersss = \App\Models\User::where("id", "!=", Auth::id())->get()->map->only("id");
    foreach ($usersss as $userrss) {
        $userIds[] = $userrss["id"];
    }
@endphp
...
<body>
    ...
    <p class="display-1 p-0 m-0">{{ Auth::user()->name }}</p>
{{--    {{ dd($currentUser->messages($userToChat["id"])) }}--}}
                    ...
                    <div class="card-body contacts_body">
                        <ul class="contacts">
                            @foreach ($users as $user)
                                <li class="">
                                    <a href="{{ route('conversation', $user['id']) }}" style="text-decoration: none">
                                        <div class="d-flex bd-highlight">
                                            <div class="img_cont">
                                                <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg"
                                                    class="rounded-circle user_img">
                                                <span class="user-status" id="userStatus-{{ $user['id'] }}"></span>
                                                {{-- <span class="online_icon"></span> online_icon offline --}}
                                            </div>
                                            <div class="user_info">
                                                <span>{{ $user['name'] }}</span>
                                                <p class="user_status" id="user_status-{{ $user["id"] }}"></p>
                                            </div>
                                        </div>
                                    </a>
                                </li>
            ...
            <div class="col-md-8 col-xl-6 chat">
                <div class="card">
                    <div class="card-header msg_head">
                        <div class="d-flex bd-highlight">
                            <div class="img_cont">
                                <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg"
                                    class="rounded-circle user_img">
                                <span class="online_icon offline" id="userToChat-{{ $userToChat['id'] }}"></span>
                            </div>
                            <div class="user_info">
                                <span>Chat with {{ $userToChat['name'] }}</span>
                                {{-- <p>1767 Messages</p> --}}
                            </div>
                            {{-- <div class="video_cam">
                    ...
                    <div class="card-body msg_card_body">
                        @foreach($currentUser->messages($userToChat["id"]) as $messages)
                            ...
                            @else
                                <div class="d-flex justify-content-start mb-4">     {{-- User to chat msg --}}
                                    <div class="img_cont_msg">
                                        <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg"
                                             class="rounded-circle user_img_msg">
                                    </div>
                                    <div class="msg_cotainer" id="msg-container">
                                        {{ $messages["content"] }}
    ...
    <script>
{{--        const ab = {{ $users->get("id") }};--}}
//         console.log(ab);

        const userIds = {{ json_encode($userIds) }};
        ...
        socket.on("connect", () => {
            socket.emit("user_connected", {{ Auth::id() }})
        })
        ...
        socket.on("updateUserStatus", onlineUsers => {
            ...
            $(".user-status").each((i, element) => {
                element.classList.add("online_icon", "offline")
            })

            $(".user_status").text("Offline")

            const userToChatId = {{ $userToChat['id'] }};
            $(`#userToChat-${userToChatId}`).addClass("offline");

            for (const uId in onlineUsers) {
                const userStatus = $(`#userStatus-${uId}`);
                userStatus.removeClass("offline");

                if (uId == userToChatId) {
                    // console.log(userToChatId)
                    $(`#userToChat-${userToChatId}`).removeClass("offline")
                }

                for (let i = 0; i < userIds.length; i++) {
                    if(uId == userIds[i]) {
                        $(`#user_status-${uId}`).text("Online")
                    }
                }
            }
        ...
        const sendMessage = () => {
            ...
            $.ajax({
                ...
                success: response => {
                    ...
                    socket.emit("sendMsgToSv", response)
                }
            })
            ...
            {{--    receiverId: {{ $userToChat["id"] }},--}}
            {{--    message: msgElement.value,--}}
            {{--    sendingTimeDate: new Date()--}}
            {{--}--}}

            // socket.emit("sendMsgToSv", msg)
